            ...
            <main className="mt-8">
                <div className="relative mb-6">
                    ...
                    <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
                        <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 ${theme.colors.mutedText}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                {filteredProfessors.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {filteredProfessors.map((professor) => (
                            <ProfessorCard key={professor.fullName} professor={professor} universityName={scheduleData.universityName} />
                        ))}
                    </div>
                ) : (
                    <div className={`mt-6 text-center ${theme.colors.cardBg} p-8 rounded-xl shadow-sm`}>
                        ...
                        <p className={`mt-1 text-sm ${theme.colors.mutedText}`}>
                            {searchTerm ? 'Попробуйте изменить поисковой запрос.' : 'Для этого факультета нет данных о преподавателях.'}
                        </p>
                    </div>
                )}
            </main>
