...
const PairedClassCard: React.FC<PairedClassCardProps> = ({ time, sessions, commonDetails, currentAcademicWeek }) => {
    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden flex transition-all duration-300 ease-in-out  lg:hover:shadow-lg lg:hover:-translate-y-1">
      ...
      <div className="p-5 flex flex-col md:flex-row items-start md:items-center gap-4 w-full">
        ...
        <div className="w-full md:w-3/4 space-y-2">
          ...
          {commonDetails && (sessionFirst || sessionSecond) && <div className="border-t border-slate-200 mx-4"></div>}

          {sessionFirst && (
            <WeekRow weekLabel="1-ая неделя" details={sessionFirst} isCurrent={currentAcademicWeek === "first"} />
          )}
          {hasBothAlternating && <div className="border-t border-slate-200 mx-4"></div>}
          {sessionSecond && (
            <WeekRow weekLabel="2-ая неделя" details={sessionSecond} isCurrent={currentAcademicWeek === "second"} />
          )}
        </div>
      </div>
    </div>
